...
- 如果 `.strm` 指向媒体库目录之外但仍属于同一网盘挂载，例如媒体库是 `/NetDisk/115-Strm`、目标是 `/NetDisk/CloudNAS/...`，需要在服务端设置 `STRM_LOCAL_ROOTS=/NetDisk`；该目录必须在容器内真实挂载，并且 `.strm` 中的路径必须使用容器内路径。
...
2. 保留或重新应用后端 STRM 文件：
   ...
   - Docker 部署时确认 `.strm` 目标路径已挂载到容器内相同路径；跨库目标通过 `STRM_LOCAL_ROOTS` 显式声明，不能只挂载宿主机目录而不映射容器路径。
...
5. 手工验证三件事：
   ...
   - 容器内执行 `ls /NetDisk/...` 能看到 `.strm` 指向的目标文件，并确认服务端环境变量包含 `STRM_LOCAL_ROOTS=/NetDisk`。
