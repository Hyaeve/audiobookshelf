...
本文记录本地定制功能，方便后续同步上游 AudioBookShelf 更新。定制功能包括 STRM 支持、全局补全队列、扫描机制、书籍匹配与章节排序、计划任务、主题切换及中文搜索增强。
...
- 所有 STRM 媒体预读入口共用一个全局书籍队列，同时最多预读一本书。队列按“播放触发 > 手动执行 > 计划任务”优先级选择下一本书；同一优先级内按请求进入队列的先后顺序处理。当前正在预读的书不会被抢占，完成后才重新选择高优先级队列。播放触发的后台预读和所有手动预读共用所属媒体库设置中的“媒体预读 QPS”（`strmMetadataQps`，默认 2.0），并统一在每预读 3000 个文件后暂停 5 分钟；计划任务媒体预读仍使用服务端设置中的独立 QPS 与批量阈值。已有完整音轨信息的目标不会重复请求。
...
- 四个入口统一固定“每预读 3000 个文件后暂停 5 分钟”。播放触发预读**不再有**每本书完成后额外固定暂停的冷却，节流完全由 QPS 与 3000 文件批量暂停控制。
...
- 选择多本书籍媒体预读时在选中书籍之间共享同一个 `throttleState`，跨书累计每 3000 个文件暂停 5 分钟；媒体库级手动预读在整个媒体库范围内累计计数。多本书籍会按提交顺序依次进入全局预读队列，不会并发扫描。
...
- 播放触发的媒体预读通过 [`completeStrmBookAfterPlayback`](../server/managers/PlaybackSessionManager.js:666) 以 `playback` 优先级入队，走 `useLibraryQps: true` 读取所属媒体库的 `strmMetadataQps`，每 3000 文件暂停 5 分钟，并用 `strmCompletionQueuedIds` 按书籍 ID 去重。
- 手动入口以 `manual` 优先级入队，全部使用 `useLibraryQps: true`：单本 [`completeStrmItem`](../server/managers/PlaybackSessionManager.js:951)、多本 [`completeStrmItems`](../server/managers/PlaybackSessionManager.js:1091)（共享 `throttleState`）、媒体库级 [`completeStrmLibrary`](../server/managers/PlaybackSessionManager.js:883) 与 [`_completeStrmLibrary`](../server/managers/PlaybackSessionManager.js:893)。手动入口通过 `strmManualEnqueueChain` 串行准备作业，避免并发重复提交。
- QPS 与批量参数集中在文件顶部常量 `DEFAULT_STRM_METADATA_QPS = 2.0`、`STRM_METADATA_BATCH_SIZE = 3000`、`STRM_METADATA_PAUSE_MINUTES = 5`（[`PlaybackSessionManager.js:24`](../server/managers/PlaybackSessionManager.js:24)），媒体库取值统一走 [`getLibraryStrmQps`](../server/managers/PlaybackSessionManager.js:591)（越界或非法回落默认 2.0）。[`completeStrmBook`](../server/managers/PlaybackSessionManager.js:732) 在 `options.useLibraryQps` 为真时按媒体库取 QPS，否则回落 `options.qps`，并把实际 `requestIntervalMs` 回写到共享的 `throttleState`。
...
- 本地新增的第二项“书籍匹配”横条使用 `ai-book-match` 任务动作。它支持 cron、图书媒体库多选、默认关闭的“全局匹配”、默认关闭的“入库匹配”和 0.5 小时步长的时间限制；设置窗口为左右双栏，左侧是任务参数，右侧是 OpenAI 兼容接口地址、API 密钥、模型和自动应用最低置信度。“全局匹配”和“入库匹配”两个复选项固定排在左栏最底部，并在同一行左右并列对齐（左栏 `section` 使用 `flex flex-col`，复选项容器用 `flex flex-wrap items-center mt-auto pt-5 gap-x-8 gap-y-2` 压到底，窗口窄时自动换行），功能说明不再用小字段落，而是与媒体库编辑窗口设置栏一致的 `material-symbols` 感叹号图标悬浮显示，且 `ui-tooltip` 只包裹感叹号图标。全局匹配关闭时只处理未匹配图书；开启后所选媒体库中的全部有效图书都会重新经过书籍匹配，并以覆盖模式应用匹配元数据。入库匹配开启后由本任务的匹配逻辑接管新书籍扫描入库后的匹配操作。横条第二行在手动或计划任务完成后均显示上次执行时间、耗时和成功匹配的图书数量，并区分“上次计划执行”与“上次手动执行”。AI 接口未配置时任务仍可运行，只使用本地提取规则和全称兜底。
- 书籍匹配设置新增“元数据匹配”多选项（配置字段 `aiBookMatchOverrideFields`），默认把 16 个字段全部纳入，可删除也可重新添加。该选项只管“是否覆盖已有值”：字段在列表内时，匹配成功后可以覆盖书籍已有内容；字段被删除时，已有值保持不变，但书籍原本为空时仍然写入匹配到的内容（例如删除“封面”后，有封面的书不被替换，无封面的书仍会下载封面）。元数据锁优先于该选项，被锁字段即使在列表内也不会被计划任务写入。
- “补全元数据”横条新增“元数据补全”多选项（配置字段 `bookMetadataCompletionFields`），默认全选，可删可加。未选中的字段完全不写入（即便书籍该字段为空）；选中的字段仍照旧逻辑只补缺、不覆盖。两个多选项共用同一张字段表，服务端定义在 [`server/utils/bookMetadataFields.js`](../server/utils/bookMetadataFields.js:7)，前端对照表在 [`METADATA_FIELD_OPTIONS`](../client/pages/config/scheduled-tasks.vue:130)，两边须保持同步；服务端保存时过滤未知键并按规范顺序去重，非法字段返回 400。
- “元数据匹配”和“元数据补全”不使用上游的 `ui-multi-select-dropdown`，而是本地新增的内联开关组件 [`widgets-metadata-field-selector`](../client/components/widgets/MetadataFieldSelector.vue:1)。原因：`MultiSelectDropdown` 的下拉菜单会被 [`unmountMountMenu`](../client/components/ui/MultiSelectDropdown.vue:83) 移到 `document.body` 并由 [`recalcMenuPos`](../client/components/ui/MultiSelectDropdown.vue:76) 固定定位到输入框下方（永远向下、不会向上翻转，与 [`MultiSelect.recalcMenuPos`](../client/components/ui/MultiSelect.vue:148) 不同），而计划任务设置窗中这两个选择框本身已接近窗口底部，菜单弹出后落在可视区之外。新组件完全不弹出任何层：16 个字段直接以胶囊按钮形式平铺在原框内（`flex-wrap` + `gap: 0.375rem`），点击即切换选中状态；默认全选，选中项用 `--abs-theme-accent` 色高亮（`.metadata-field-chip`），未选项变为置灰（`.metadata-field-chip-off`，灰蓝边框 + 灰字），因此选与不选一眼可辨。按钮带 `aria-pressed`，组件只向外 `input` 规范顺序的字段数组（按 `items` 顺序过滤），与服务端 `normalizeBookMetadataFields` 的顺序一致。上游组件 `MultiSelectDropdown.vue` 本身未修改，不新增上游耦合点。
- 计划任务设置窗内所有感叹号悬浮提示的触发区域只包裹感叹号图标本身。[`Tooltip.setTooltipPosition`](../client/components/ui/Tooltip.vue:78) 是以 `$refs.box`（即 `ui-tooltip` 的根 div）的 `getBoundingClientRect()` 为基准、默认 `direction: 'right'` 定位到盒子右侧的。之前把整个文字标签或 `<label>` 包进 `ui-tooltip` 内，导致两个问题：一是悬浮在文字上（而不是感叹号上）就会弹出提示；二是基准盒子包含整行文字，右侧定位会把提示框推到设置窗右边缘。现在结构改为 `<div class="flex items-center"><span>标题</span><ui-tooltip class="inline-flex items-center ml-1"><span class="material-symbols icon-text text-sm">info</span></ui-tooltip></div>`，共四处：元数据匹配、元数据补全、全局匹配、入库匹配。复选框的 `<label for>` 仍在 `ui-tooltip` 外部，点文字依旧可切换开关。注意：`ui-tooltip` 根元素默认是块级 div，必须给它 `inline-flex items-center` 才不会在 flex 行内被拉高，否则基准盒子变高、提示框垂直居中位置会偏。
- 本地新增的“补全元数据”横条使用 `book-metadata-completion` 任务动作，支持 cron、图书媒体库多选和 0.5 小时步长的时间限制。该任务与 AI 书籍匹配严格分离：按照每本书所属媒体库的默认 `library.provider` 逐本请求 [`BookFinder.search()`](../server/finders/BookFinder.js:374)，找到候选后调用 [`Scanner.quickMatchLibraryItem()`](../server/scanner/Scanner.js:88)；始终关闭覆盖封面和覆盖详情，只补充缺失字段。停止时 provider 搜索等待可被协作式取消抢先结束，取消后不继续写入或处理下一本书。每本书的日志只显示书名、提供商和实际补全的字段类别（如标题、作者、流派、出版商、系列、封面），不输出元数据具体内容；自定义提供商显示设置中配置的名称，不显示 `custom-UUID`，内置提供商显示规范可读名称。AI 书籍匹配仍只处理 [`getUnmatchedCandidates()`](../server/managers/AiBookMatchManager.js:100) 判定为未完成匹配的书籍，对已匹配书籍不执行 AI 请求、provider 搜索或写入。
...
- [`AiBookMatchManager`](../server/managers/AiBookMatchManager.js:42) 以书籍的 `media.title`（该字段就是原始文件夹名）为输入。本地规则命中时把该名称连同已确认的书名发送给 OpenAI 兼容接口 `/chat/completions`，只让 AI 提取作者和演播者；本地规则都不适用时才让 AI 同时提取书名、作者和演播者；作者与演播者去重合并后分别作为既有 [`BookFinder.search`](../server/finders/BookFinder.js:374) 的标题和作者参数，再获取最多 8 个候选。随后 AI 只能返回本次候选数组中的序号、0 至 1 置信度和理由；服务端拒绝越界序号、非法 JSON 与非法置信度，禁止 AI 自由生成并直接写入元数据。
- 达到 `aiBookMatchConfidence` 阈值后，任务调用 [`Scanner.applyBookMatch`](../server/scanner/Scanner.js:188)，与原快速匹配共用封面、作者、系列、元数据文件和 Socket 更新流程。
- 书名与作者按固定优先级逐级尝试，任一级搜索并确认匹配成功即结束，不再执行后续级别（[`buildMatchAttempts`](../server/managers/AiBookMatchManager.js:277)、[`matchLibraryItem`](../server/managers/AiBookMatchManager.js:480)）。**书名始终由命中的最高优先级规则决定，AI 只在本地规则命中时负责补作者栏**：
  1. 本地书名号（最高优先级）：原名称含 `《》`、`「」` 或 `『』` 时直接取括号内文本作为书名。作者栏分两种情况：已配置 AI 时由 AI 提取（演播）作者填入作者栏（规则键 `quoted-ai-author`）；未配置 AI（或 AI 熔断/失败）时，把完整标题里**除书名号及其内容以外的剩余文本**填入作者栏（规则键 `quoted-local-author`）。
  2. 本地符号分隔：无书名号时按 `丨`、`|`、`｜`、`.`、`．`、`-`、`－`、`—`、`–` 定位第一个符号，取符号之前的片段作为书名。作者栏同样分两种情况：已配置 AI 时由 AI 提取（演播）作者（规则键 `separator-ai-author`）；未配置 AI 时把**第一个符号之后的全部文本**填入作者栏（规则键 `separator-local-author`）。
  3. 无作者重试：第一、二级带作者搜不到或未达置信度时，去掉作者栏、仅用同一个本地书名再搜一次（规则键 `quoted` / `separator`）。这一步只是提高命中率的补充，书名仍来自本地规则。
  4. AI 书名 + 人物：既无书名号也无符号标识且已配置 AI 时，由 AI 分别提取书名和（演播）作者，同时填入书名栏和作者栏搜索（规则键 `ai-title-author`）。
  5. AI 仅书名：第四级搜不到或未达阈值时，作者栏留空，仅用 AI 提取的书名再搜一次（规则键 `ai-title`）。
  6. 全称兜底（最低优先级）：书名号、符号标识、AI 接口配置都没有（或以上全部未匹配）时，直接把完整原名称放入书名栏搜索（规则键 `full-name`）。
- 本地规则命中时**只把 AI 用于提取（演播）作者，绝不让 AI 改写书名**：[`extractSearchMetadata`](../server/managers/AiBookMatchManager.js:318) 会把 [`extractLocalTitleWithRule`](../server/managers/AiBookMatchManager.js:191) 得到的书名作为 `confirmedTitle` 传给 AI，并要求原样复制回 `title`，因此书名号和符号分隔两级的书名恒等于本地提取结果。AI 请求失败时由 [`extractSearchMetadataSafely`](../server/managers/AiBookMatchManager.js:254) 吞掉异常，自动退回本地作者规则，不影响该书继续匹配。
- 未配置 AI 时整条链路只有本地规则（含本地作者）和全称兜底，候选判断退化为直接采用 provider 的首个候选，审计状态记为 `matched-local`。
- 本地作者由 [`extractLocalAuthor`](../server/managers/AiBookMatchManager.js:215) 推导：`quoted` 规则去掉 `《书名》` 整段后取剩余文本，`separator` 规则取第一个分隔符之后的全部文本；两者都用 [`trimLocalAuthor`](../server/managers/AiBookMatchManager.js:199) 去掉首尾的分隔符、空白和标点，并截断到 300 字符。剩余文本为空（例如整名就是 `《天启之门》`）时不追加带作者的尝试。
- 已配置 AI 时每一级仍由 AI 候选判断把关，达到 `aiBookMatchConfidence` 阈值才写入元数据；未达阈值的级别继续尝试下一级，全部失败后按最后一次失败原因写入 `unmatched` 或 `needs-review` 审计。
- 匹配审计新增 `rule` 字段记录命中的提取规则，日志额外输出中文规则名称（书名号、书名号+AI 人物、书名号+剩余文本、符号分隔、符号分隔+AI 人物、符号分隔+后段文本、AI 书名+人物、AI 仅书名、全称）。
- “入库匹配”是书籍匹配设置中的可选项，配置字段 `aiBookMatchOnScan`，默认关闭。勾选后**完全接管**新书入库的匹配：扫描器会跳过自己的 provider 匹配步骤（即 `scannerFindCovers` 控制的封面搜索，判定点在 [`BookScanner.scanNewBookLibraryItem`](../server/scanner/BookScanner.js:447) 内的 [`bookMatchHandlesScan`](../server/scanner/BookScanner.js:567)，它调用 [`willHandleScanMatch`](../server/managers/AiBookMatchManager.js:582)），改由 [`enqueueScanMatch`](../server/managers/AiBookMatchManager.js:596) 把新书交给上述书籍匹配逻辑：入队后串行处理，一次只匹配一本书，按书籍 ID 去重；只处理书籍媒体库中的新项目，并在书籍匹配设置里选择了媒体库时仅处理所选媒体库（未选择媒体库时不限制）。不勾选时 `willHandleScanMatch` 返回 `false`，扫描器完全走上游原有匹配机制，不会多跑一轮书籍匹配。入库匹配同样遵守“元数据匹配”多选项。完整扫描与 watcher 增量扫描共用同一入口 [`scanNewLibraryItem`](../server/scanner/LibraryItemScanner.js:186)，因此两种扫描方式行为一致。
...
- AI 提取超时或失败时该级只丢掉作者栏（退回本地作者规则或无作者重试），不阻断整本书的匹配。未配置 AI 时 [`runAiBookMatch`](../server/managers/CronManager.js:245) 不再抛出“未配置”错误，任务可以只跑本地规则。
- 计划任务停止通过 [`AbortController`](../server/managers/CronManager.js:257) 中断当前 AI HTTP 请求，并由 [`throwIfAborted`](../server/managers/AiBookMatchManager.js:408) 在每一级 provider 搜索和候选判断前后检查停止状态；取消不会写入 `needs-review` 审计，也不会继续处理下一级或下一本书。
- 计划任务每批读取书籍后先调用 [`getUnmatchedCandidates`](../server/managers/AiBookMatchManager.js:100) 预过滤，只有“除标题、描述和扫描基础信息外，所有扩展元数据均为空”的历史书籍才进入逐本匹配。持续时间、文件大小、音轨、章节、文件路径和 `libraryFiles` 属于扫描基础信息，不影响候选资格。
...
- 所有计划任务横条的设置窗口把 Cron 表达式输入框和时间限制输入框放在同一行、左右各占一半并顶部对齐（样式类 `.scheduled-task-field-grid`，窗口宽小于 640px 时自动变单列）；无时间限制的“清理丢失项目”只显示 Cron 输入框。
- 所有计划任务横条的设置窗口把执行时长输入框的标签统一为“时间限制（h）”（原为“单次最长执行时间（小时）”），共三处模板文案：书籍匹配左栏、扫描/补全/预读共用块、`hasMaxHours` 兜底块；[`validateDrafts`](../client/pages/config/scheduled-tasks.vue:282) 的校验提示文案也同步为“时间限制（h）必须是不小于 0.5 且为 0.5 倍数的数值”，保持提示与标签一致。
...
- 五条横条的设置窗口保存前先做前端数值校验并始终给出提示，避免出现“点保存没反应”的静默失败。[`validateDrafts`](../client/pages/config/scheduled-tasks.vue:282) 依次校验时间限制（h）（≥0.5、0.5 倍数）、扫描 QPS（0.1–10、0.1 倍数）、暂停阈值（≥500、500 倍数整数）和自动应用最低置信度（0.5–1）；任一不合法就用 `$toast.error` 提示中文原因并直接返回，不发起 PATCH。数值转换走 [`draftNumber`](../client/pages/config/scheduled-tasks.vue:275)（`v-model.number` 在输入框被清空时会得到 `null`/`""`），步长判定走 [`isStepMultiple`](../client/pages/config/scheduled-tasks.vue:279) 的 1e-9 容差比较，避免 `2.5`、`0.3` 一类小数被浮点误差误判。
- [`saveSettings`](../client/pages/config/scheduled-tasks.vue:304) 的 `catch` 必须把服务端返回的错误文本（`error.response.data`）通过 `$toast.error` 显示出来，没有文本时退回“保存设置失败”；成功时显示“<任务名>设置已保存”。不能只 `console.error`，否则保存失败在界面上只表现为“弹窗不关闭”。同时 `draftAiUrl`/`draftAiModel`/`draftAiKey` 一律用 `(this.draftAiX || '').trim()` 取值，防止 draft 为 `null` 时 `TypeError` 被 `catch` 吞掉造成静默失败。
...
- 根因不在提取规则，而在 AI 请求的异常处理。以 `恶魔法则.演播一种侃侃.跳舞.2023` 为例：名称含 `.`，本地符号分隔规则正常命中并提取出 `恶魔法则`，provider 也返回了候选；随后 [`chooseCandidate`](../server/managers/AiBookMatchManager.js:365) 请求 AI 接口拿到 `503`，`axios` 抛出的异常一路冒泡出 [`matchLibraryItem`](../server/managers/AiBookMatchManager.js:480)，被 [`CronManager`](../server/managers/CronManager.js:36) 的 `catch` 兜住后统一写成 `needs-review`。该 `catch` 构造的 `matchResult` 只有 `status` 和 `reason`，没有 `rule`/`searchTitle`/`searchAuthor`，所以日志把这三项打成 `-`，造成“规则没生效”的误判。入库匹配走 [`processScanMatchQueue`](../server/managers/AiBookMatchManager.js:606) 的 `catch`，表现为“入库匹配失败”。
- 修复一：AI 请求统一走带重试的 [`postAiRequest`](../server/managers/AiBookMatchManager.js:443)。`408`、`429`、`5xx` 和网络层错误（无 `response`）最多重试 3 次；有 `Retry-After` 头时按该值等待（上限 30 秒），否则按 2 秒、4 秒递增。`401`、`403`、`400` 一类确定性错误立即失败，不做无意义重试。等待走 [`wait`](../server/managers/AiBookMatchManager.js:422)，可被停止信号打断。
- 修复二：AI 失败不再让整本书失败。[`buildMatchAttempts`](../server/managers/AiBookMatchManager.js:277) 的书名提取失败原本已经降级，本次补上候选判定：`matchLibraryItem` 内的 `chooseCandidate` 失败后只把本书剩余级别的 `aiUsable` 置为 `false`，直接采用 provider 首个候选并把审计写成 `matched-local` / `source: local`，而不是抛出异常。审计里的 `status`/`source`/`model` 改为依据实际是否用到 AI 的 `usedAi` 变量，而不是“是否配置了 AI”的 `aiConfigured`，避免降级后仍被标成 `matched-ai`。
- 修复三：新增软熔断。连续 `AI_FAILURE_STREAK_LIMIT = 3` 次 AI 传输失败后，[`noteAiFailure`](../server/managers/AiBookMatchManager.js:81) 把 `aiUnavailableUntil` 推后 `AI_COOLDOWN_MS = 5` 分钟，期间 [`isAiUsable`](../server/managers/AiBookMatchManager.js:66) 返回 `false`，整批书籍跳过 AI 步骤、只用本地规则继续匹配，不会每本书都白等三次重试超时。任一次 AI 成功即由 [`noteAiSuccess`](../server/managers/AiBookMatchManager.js:70) 清零。熔断状态只存在于 `AiBookMatchManager` 实例字段，不落库、不影响 `isConfigured` 的语义。
- 取消判定集中到 [`isCancelledError`](../server/managers/AiBookMatchManager.js:412)（检查 `options.signal.aborted`、`ERR_CANCELED`、`CanceledError`），停止计划任务时仍立即中断且不写失败审计、不触发熔断计数以外的重试。
- 修复四：兜底日志不再丢信息。[`CronManager`](../server/managers/CronManager.js:36) 的 `catch` 和 [`processScanMatchQueue`](../server/managers/AiBookMatchManager.js:606) 的 `catch` 都会重新用 `extractLocalTitleWithRule` 算出规则与书名后再写审计和日志，因此即便出现未预料的异常，日志也会显示真实的提取规则和搜索标题，而不是三个 `-`。
...
- 顶栏五个图标按钮的 tooltip 现在统一对齐。统计数据、上传、设置三个入口本来就是外层 `w-8 h-8 flex items-center justify-center mx-1` + 内层 `<ui-tooltip direction="bottom" class="flex items-center">`；主题切换器原来是 `mx-1 sm:mx-2` 容器加 `w-9 h-9` 按钮，活动小铃铛原来是 `w-4 h-4 mx-3` 容器加 `w-10 sm:w-full` 按钮，两者的悬浮框基准盒尺寸和外边距都与其余三个不同，导致提示框高低和左右不齐。现已把两者的容器都改为 `w-8 h-8 ... mx-1`、按钮改为撑满容器，因此 [`Tooltip.setTooltipPosition`](../client/components/ui/Tooltip.vue:78) 计算出的 `boxRect` 在五个入口完全一致。
- 仅把容器改成 `w-8 h-8` 还不够：[`Tooltip.setTooltipPosition`](../client/components/ui/Tooltip.vue:78) 用的基准盒是 `ui-tooltip` 自己的根 div（`$refs.box`），`direction: 'bottom'` 时 `top = boxRect.top + boxRect.height + 4`、`left` 按基准盒水平居中。统计/上传/设置三个入口的 `ui-tooltip` 只包着 `text-2xl` 图标（高 24px），而主题切换器原本把 `ui-tooltip` 包在 `w-8 h-8` 按钮外面（高 32px），活动铃铛的 `class="flex items-center"` 也会被父级 `h-full` 拉成整个按钮高度，两边基准盒都比参考入口高 8px，提示框就会多往下推 8px。现在两处都把 `ui-tooltip` 的基准盒固定为 `w-6 h-6 flex items-center justify-center`（与 24px 图标同尺寸）：[`ThemeSwitcher.vue`](../client/components/app/ThemeSwitcher.vue:4) 把 `ui-tooltip` 改到 `<button>` 内部只包 `<img class="w-6 h-6">`（不能包在按钮外），[`NotificationWidget.vue`](../client/components/widgets/NotificationWidget.vue:5) 的 `tasksRunning` 和非 `tasksRunning` 两个分支同步换成同一组尺寸类（旋转动画的 `la-sm` 是 16px，也靠这个固定盒子对齐）。结论：上栏 tooltip 对齐的关键不是容器尺寸，而是 `ui-tooltip` 根元素本身的盒子尺寸必须在五个入口间一致。
...
### 14. 2026-09-15 编辑窗口与计划任务窗口滚动条样式

- 编辑书籍窗口的简介输入框和计划任务设置窗口统一使用细浅滚动条：宽度为 `6px`，滑块采用半透明浅灰色，轨道透明，减少对内容的视觉干扰。
- 同时覆盖计划任务设置窗口内的媒体库候选列表；仅调整滚动条外观，不改变滚动区域、弹窗高度或鼠标滚轮行为。
- 简介输入框右下角拉伸器保持两条平行斜线的示例样式：上方斜线较长、下方斜线较短，且仍支持拖动改变输入框高度。

### 15. 2026-09-16 中文搜索增强（基于 a71406b 重新实现）

#### 界面、开关与使用方法

- 在计划任务页面「书籍匹配」之后增加「中文搜索增强」，小字描述固定为“支持中文模糊搜索和拼音搜索”。设置包括 Cron、时间限制（h）、图书媒体库多选、标题/副标题/作者/演播者四项胶囊候选。四项默认全不选，Cron 默认空，时间限制默认 1 小时且按 0.5 小时递增。
- 第一次选择媒体库和字段并保存后，手动执行一次为已有书籍建立索引；新增加字段或媒体库范围后再次执行补齐。后续入库、匹配完成、编辑元数据自动后台增量更新。Cron 非必需，留空不注册定时任务，不影响增量更新；设置 Cron 可定期校准因异常中断遗漏的索引。
- 可手动停止，活动中显示进度；保存服务端上次运行时间、耗时、更新书籍数，区分手动与计划执行。未选字段或媒体库时手动执行返回中文提示。允许保存全部不选以关闭功能，原有普通搜索仍然可用。
- 支持中文连续片段、全拼、首字母、全拼与首字母混合、中文与拼音混合：`cfwlzcqrs`/`cfwl`/`chongfwl`/`重返wl` 匹配「重返五零之传奇人生」，`dscy` 匹配标题片段「都市穿越」，`wangk` 匹配「王凯」「王奎荣」。支持逐字多音候选、全角/大小写/声调规范化；不跨作者名称、元数据字段或标题竖线片段拼接。
- “模糊”指片段与拼音匹配，不代表语义搜索或任意拼写纠错。「望古」应由 `wangg`/`wanggu` 命中，而不是人为将 `g`、`k` 等同；若其已选作者/演播者匹配 `wangk`，可以按这些字段召回书籍。
- 增强接入 `/api/libraries/:id/search` 的图书结果：普通原文、ISBN/ASIN 命中优先，拼音结果去重后填充剩余名额。按作者/演播者命中返回对应书籍，不改造作者/演播者分类建议列表，也不改变在线提供商匹配、播客搜索或书籍匹配优先级。

#### 索引存储与增量维护

- 独立模型 `server/models/ChineseSearchIndex.js` 在**现有 SQLite 数据库文件**中新增 `chineseSearchIndices` 表；使用既有模型初始化与 `sync({ alter: false })` 创建，不修改原有表结构，不另建数据库文件，不改写原始元数据或 metadata 文件。索引随原数据库备份，可由任务重建。
- 每本书一条索引：`libraryItemId` 主键、`libraryId`、SHA-256 `fingerprint`、JSON `payload`、创建/更新时间；媒体库与项目 ID 的组合索引用于游标分页。指纹包含索引版本、媒体库和当前选中字段的原始值，未变化不重写。
- `server/utils/chineseSearch.js` 使用固定版本 `pinyin-pro@3.29.4`（MIT、本地纯 JavaScript 拼音字典）。无需训练分词器、调用 AI/网络服务或探测媒体文件。仅保存每字候选读音，查询时通过状态集合识别全拼/首字母组合，不穷举所有组合。
- 搜索每批读取 200 行索引、批间让出事件循环；有效查询最长 128 个规范化字符。命中后读取当前书籍，核验媒体库、缺失/无效标记、用户敏感内容与标签权限，并检查指纹，避免短暂陈旧索引返回旧标题。取消字段/媒体库后旧缓存不再参与增强搜索，普通搜索不受影响。
- 索引批量任务按 ID 游标每批 50 本，边处理边持久化，下一次运行跳过指纹一致的书籍。任务有执行互斥、停止和时间限制，当前单书操作完成后响应停止，已落库结果保留。任务错误标为失败，不会一直停在运行中。
- `ChineseSearchManager.installHooks` 注册 `libraryItem`、`book`、`author`、`bookAuthor` 的保存、批量修改、新建和删除钩子；提交事务后才加入去重队列，回滚不更新；作者改名、关联增删、匹配写入均会触发。按书籍 ID 串行化写入，避免批量任务与自动更新互相覆盖。
- 仅更新音轨时长/编码等未选字段不触发拼音更新，避免媒体预读引入额外写入。后台队列不阻塞扫描/匹配；索引监听或构建异常记录日志，原始元数据操作仍可成功，后续任务可修复。自动更新是最终一致性，不承诺保存返回时索引已就绪。
- 删除/缺失项目清理对应索引，正常结束的任务清理孤立记录。全部关闭时暂存索引不使用，不强制大批删除；重新开启后建议运行校准。突然中断可能丢失尚未执行的内存队列，手动任务或可选 Cron 用于补漏。
- 性能边界：单本短文本转换轻量，但首次建库速度受总量及数据库读写影响；当前没有完整倒排检索引擎，无命中查询仍遍历选中媒体库的索引候选。未来超大库可加入倒排预筛选，不能把纯拼音转换速度当作实际搜索延迟。

#### 设置白名单与上游解耦

- `ServerSettings` 增加 `chineseSearchCronExpression`、`chineseSearchLibraryIds`、`chineseSearchMaxHours`、`chineseSearchFields` 四项普通设置白名单及对应默认值/加载/序列化；`chineseSearchLastRun` 仅服务端写入，禁止普通设置请求覆盖。现有定制配置从 24 项增加为 28 项。
- `MiscController.updateServerSettings` 校验字段、图书库 ID、Cron、时间限制；`ApiRouter` 增加管理员专用 `POST /api/chinese-search/run` 和 `/stop`；`CronManager.updateChineseSearchCron` 负责空值停用、表达式变化时替换定时任务，执行逻辑独立在 `ChineseSearchManager.js`。
- 最小耦合点：`Database.buildModels` 的新模型/钩子注册，`libraryItemsBookFilters.search` 的增强结果补充，`scheduled-tasks.vue` 的横条、设置和运行记录映射。界面复用现有 `MetadataFieldSelector` 胶囊和 `Tooltip` 感叹号说明。
- 上游同步时优先保留独立模型、管理器及纯函数文件，再适配上述耦合点；更改索引结构或拼音规则应递增 `INDEX_VERSION` 并运行任务。不能跳过权限核验、设置白名单测试或直接覆盖核心搜索实现。
- 本轮确认基线为 `a71406b`；用户给出的 StrmAssistant 路径当前不存在，因此本实现依据已确认方案独立完成，没有声称复用无法访问的参考源码。未包含之前中断的 HTTP 代理改动。

#### 本轮验证

- 后端完整 `npm test -- --reporter dot`：**511 passing**；其中新增及扩展的搜索/增量维护/Cron/设置相关测试组合为 82 项。覆盖真实模型和搜索接口、权限、改名、关联增删、事务提交/回滚、普通结果优先、字段关闭、默认不开启、取消/超时和失败后重跑。
- 前端 `npm --prefix client run generate` 成功；三个改动 Vue 模板编译、任务顺序/指定小字、四项胶囊默认未选、Cron 留空及 API 映射检查通过。依赖与锁文件一致，`git diff --check` 通过。构建保留既有 PostCSS、资源体积及 Node 弃用警告。
- 未使用生产数据库或实际网盘媒体；尚未做实际浏览器中的滚动条视觉验收。上线后确认外层编辑窗口、简介与计划任务候选列表在各主题下均为默认低可见度、悬停可见，简介右下角仍可拖动。

### 16. 2026-09-16 编辑窗口与媒体库候选框的低可见度滚动条

- 新增 `client/assets/subtleScrollbars.css`，由 `app.css` 导入。通过 `subtle-scrollbars` 容器类仅覆盖书籍编辑窗口（外层元数据表单、各标签页、简介及候选列表）和计划任务设置面板中的滚动区，不改变书架等页面的主题滚动条。
- Chromium/WebKit 下宽/高 `3px`，透明轨道、圆角滑块，默认浅灰透明度 `0.10`，鼠标悬停或键盘聚焦时 `0.28`。不隐藏滚动功能，不使用 `display: none` 或 `overflow: hidden`；Firefox 使用原生 `thin` 回退，实际像素由浏览器决定。
...
- `client/pages/config/index.vue` 在设置主页右栏“安全”下方新增“代理”：填写 HTTP/HTTPS 代理地址，新请求立即生效。支持带用户名密码的地址；仅允许 HTTP/HTTPS 协议及主机、端口，不接受路径、查询参数或片段。2026-09-17 调整为回车或失焦自动保存，留空明确直连，不再回退环境变量代理，详见第 18 节。
...
### 18. 2026-09-17 代理设置极简化与自动保存

- 设置页代理区域仅保留“代理”标题及输入框，移除副标签、占位示例、小字说明和保存按钮。输入框通过 `aria-labelledby` 关联标题，保留键盘和读屏支持。
- 回车或点击其他区域（失焦）自动保存，仅在值变化时发送请求；保存期间只读并避免重复请求。保存失败保留输入内容并提示，下次回车或失焦可重试；加载失败禁止误保存空值，重新聚焦输入框重试加载。
- 留空表示内置提供商 API 明确直连：`metadataHttpClient` 设置 `proxy: false` 并采用直连 agents，不使用 HTTP_PROXY/HTTPS_PROXY 等环境代理。配置非空时仍支持 HTTP/HTTPS 代理和 NO_PROXY；不改变全局 Axios 或其他网络功能。
- 上游耦合点仅为 `client/pages/config/index.vue` 的代理模板/状态/读写方法与 `server/utils/metadataHttpClient.js` 的空地址分支。回归覆盖清空后下一次请求直连，以及环境变量配置代理时空地址仍直连。
- 验证：完整后端测试 **555 passing**；Vue 模板编译、自动保存、回车/失焦绑定、重复提交防护、非法地址拦截、清空及失败重试逻辑检查通过。未进行实际浏览器视觉验收。
...
- [`server/managers/PlaybackSessionManager.js`](../server/managers/PlaybackSessionManager.js:666)：播放触发补全（playback 优先级、`useLibraryQps`、每 3000 文件暂停 5 分钟、书籍 ID 去重；无每本完成后固定冷却）。
- [`server/managers/PlaybackSessionManager.js`](../server/managers/PlaybackSessionManager.js:951)：单本手动媒体预读（manual 优先级、`useLibraryQps`、每 3000 文件暂停 5 分钟）。
- [`server/managers/PlaybackSessionManager.js`](../server/managers/PlaybackSessionManager.js:1091)：多本手动媒体预读（manual 优先级、`useLibraryQps`、跨书共享每 3000 文件暂停 5 分钟）。
- [`server/managers/PlaybackSessionManager.js`](../server/managers/PlaybackSessionManager.js:893)：媒体库级手动媒体预读（manual 优先级、`useLibraryQps`、累计 3000 文件暂停 5 分钟）。
...
- [`server/managers/CronManager.js`](../server/managers/CronManager.js:526)：`cancelScheduledLibraryScan` 协作式取消；计划扫描启动日志将选中的媒体库 ID 映射为名称后输出。
...
这一组锚点对应“书名与作者提取优先级链 + 入库匹配”功能，是上游同步时需要逐个确认的位置。除 `LibraryItemScanner.js` 的一行调用外，其余逻辑都集中在本地新增文件 `AiBookMatchManager.js` 内。

- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:12)：`MATCHED_AUDIT_STATUSES` 把 `matched-ai` 与本地匹配成功状态 `matched-local` 一并视为已匹配，供 [`isUnmatchedCandidate`](../server/managers/AiBookMatchManager.js:101) 判断。
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:14)：`TITLE_SEPARATOR_REGEX` 定义第二级本地规则识别的分隔符 `丨 | ｜ . ． - － — –`；紧随其后的 [`LOCAL_AUTHOR_TRIM_REGEX`](../server/managers/AiBookMatchManager.js:16) 负责裁掉本地作者文本首尾的分隔符、空白和标点。
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:22)：`MATCH_RULE_LABELS` 提取规则的中文日志名称（书名号、书名号+AI 人物、书名号+剩余文本、符号分隔、符号分隔+AI 人物、符号分隔+后段文本、AI 书名+人物、AI 仅书名、全称）。
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:45)：构造函数中的 `apiRouterCtx`、`scanMatchQueue`、`scanMatchQueuedIds`、`scanMatchRunning` 四个入库匹配队列字段，全部只存在于内存，不落库。
- [`extractLocalTitle`](../server/managers/AiBookMatchManager.js:166)：第一级书名号提取，保持原有实现不变。
- [`extractSeparatorTitle`](../server/managers/AiBookMatchManager.js:171)：第二级符号分隔提取，取第一个分隔符之前的非空片段；整名没有分隔符或首段等于原名时返回空串。
- [`extractLocalTitleWithRule`](../server/managers/AiBookMatchManager.js:191)：把两条本地规则合并为 `{ title, rule }`，书名号优先于符号分隔。
- [`trimLocalAuthor`](../server/managers/AiBookMatchManager.js:199) 与 [`extractLocalAuthor`](../server/managers/AiBookMatchManager.js:215)：未配置 AI 或 AI 不可用时的本地作者推导。`quoted` 去掉 `《书名》` 整段取剩余文本，`separator` 取第一个分隔符之后的全部文本，其他规则返回空串。
- [`extractSearchMetadataSafely`](../server/managers/AiBookMatchManager.js:254)：包裹 `extractSearchMetadata`，吞掉传输失败并记入熔断计数，只有取消信号才继续向上抛。
- [`getMatchRuleLabel`](../server/managers/AiBookMatchManager.js:231)：规则英文键到中文标签的映射，供日志使用。
- [`buildMatchAttempts`](../server/managers/AiBookMatchManager.js:277)：生成有序尝试列表。本地规则命中时书名固定为本地结果，先产出带作者的一级（AI 可用取 `*-ai-author`，否则取 `*-local-author`），再产出无作者的同名一级；本地规则都不命中且 AI 可用时才调用 [`extractSearchMetadata`](../server/managers/AiBookMatchManager.js:318) 产出 `ai-title-author`、`ai-title` 两级；最后无条件追加 `full-name`，并按“标题+作者”去重。
- [`postAiRequest`](../server/managers/AiBookMatchManager.js:443)：所有 AI 请求的唯一出口，重试 408/429/5xx 与网络错误（最多 3 次，遵守 `Retry-After`），确定性错误立即失败。
- [`isCancelledError`](../server/managers/AiBookMatchManager.js:412) 与 [`wait`](../server/managers/AiBookMatchManager.js:422)：统一的取消判定和可打断退避等待。
- [`isAiUsable`](../server/managers/AiBookMatchManager.js:66) / [`noteAiSuccess`](../server/managers/AiBookMatchManager.js:70) / [`noteAiFailure`](../server/managers/AiBookMatchManager.js:81)：AI 软熔断（连续 3 次传输失败后暂停 5 分钟，只用本地规则），常量 `AI_FAILURE_STREAK_LIMIT`、`AI_COOLDOWN_MS` 位于文件顶部。
- [`throwIfAborted`](../server/managers/AiBookMatchManager.js:408)：统一的停止信号检查点，替代原来散落的多处 `options.signal?.aborted` 判断。
- [`matchLibraryItem`](../server/managers/AiBookMatchManager.js:480)：逐级执行尝试列表。每级先 provider 搜索，无候选就进入下一级；已配置 AI 时由 [`chooseCandidate`](../server/managers/AiBookMatchManager.js:365) 把关且未达阈值继续下一级，未配置 AI 时直接取首个候选。成功后写 `matched-ai`/`matched-local` 审计并返回 `rule`、`ruleLabel`；全部失败按最后一次失败原因写 `unmatched`/`needs-review`。
- [`enqueueScanMatch`](../server/managers/AiBookMatchManager.js:596)：入库匹配入队。校验 `aiBookMatchOnScan`、`mediaType === 'book'`、`aiBookMatchLibraryIds`（为空表示不限制）与书籍 ID 去重，任一不满足直接返回 `false`，因此关闭该选项时对上游扫描流程零影响。
...
- [`test/server/managers/AiBookMatchManager.test.js`](../test/server/managers/AiBookMatchManager.test.js:70)：本地作者提取、分隔符提取、规则优先级、尝试链构建与无 AI 逐级回退用例。作者栏来源相关的用例：[`lets the AI fill only the author column`](../test/server/managers/AiBookMatchManager.test.js:96)、[`uses the text outside the book-title brackets as author`](../test/server/managers/AiBookMatchManager.test.js:110)、[`uses the text after the first separator as author`](../test/server/managers/AiBookMatchManager.test.js:124)、[`keeps the local separator title and takes only the author from AI`](../test/server/managers/AiBookMatchManager.test.js:138)、[`falls back to the local author rule when the AI author extraction fails`](../test/server/managers/AiBookMatchManager.test.js:151)。另有四个 AI 容错用例：[`retries a 503`](../test/server/managers/AiBookMatchManager.test.js:203)、[`does not retry a 401`](../test/server/managers/AiBookMatchManager.test.js:219)、[`opens the AI circuit breaker`](../test/server/managers/AiBookMatchManager.test.js:231)、[`falls back to the first provider candidate when the AI candidate decision fails`](../test/server/managers/AiBookMatchManager.test.js:248)。该测试文件在 `beforeEach` 里 stub 掉 `wait` 并调用 `noteAiSuccess()` 重置熔断，否则重试退避会让用例超时、熔断状态会在用例之间串味。
...
- [`server/managers/CronManager.js`](../server/managers/CronManager.js:245)：普通书籍匹配按 50 本分页读取后先批量过滤，只有未匹配候选进入逐本匹配流程，已匹配项直接计入 `skipped`；开启 `aiBookMatchGlobal` 后不执行该预过滤，全部有效书籍都会重新匹配。确认匹配后使用覆盖封面和详情模式写入，但总锁和字段锁继续生效。运行中通过 `AbortController` 取消 AI 请求，停止后不再写入失败审计；启动和结束日志使用媒体库名称并注明 AI 是否已配置，多个媒体库和书籍均按顺序串行处理。
...
- [`server/scanner/Scanner.js`](../server/scanner/Scanner.js:188)：`applyBookMatch` 是普通快速匹配与 AI 匹配共用的候选应用入口。
- [`server/utils/bookMetadataFields.js`](../server/utils/bookMetadataFields.js:7)：本地新增文件。`BOOK_METADATA_FIELD_LABELS` 定义 16 个字段的中文标签与规范顺序，`normalizeBookMetadataFields` 过滤未知键并去重（传入非数组时返回全部字段，用于旧配置兼容），`isValidBookMetadataFields` 供控制器校验，`getBookMetadataFieldLabels` 供日志输出中文字段名。书籍匹配的“元数据匹配”和补全元数据的“元数据补全”共用这一张表。
- [`Scanner.isFieldAllowed`](../server/scanner/Scanner.js:62)：`allowedFields` 白名单判定，非数组表示不限制；用于“元数据补全”限制哪些字段允许写入。
- [`Scanner.canOverrideField`](../server/scanner/Scanner.js:75)：先要求 `options.overrideDetails`，再检查 `overrideFields` 白名单；用于“元数据匹配”限制哪些字段允许覆盖已有值。[`quickMatchBookBuildUpdatePayload`](../server/scanner/Scanner.js:206) 内原先散落的 `|| options.overrideDetails` 全部改为调用该方法，封面判定在 `applyBookMatch` 顶部单独处理。
- [`AiBookMatchManager.willHandleScanMatch`](../server/managers/AiBookMatchManager.js:582)：入库匹配是否接管某个媒体库的判定，抽出来供 [`bookMatchHandlesScan`](../server/scanner/BookScanner.js:567) 懒加载调用以跳过扫描期封面搜索。必须懒加载 `require`，否则循环依赖会拿到空对象。
- [`AiBookMatchManager.getMatchStatusLabel`](../server/managers/AiBookMatchManager.js:241)：匹配结果状态的中文标签读取入口，映射表见 [`MATCH_STATUS_LABELS`](../server/managers/AiBookMatchManager.js:35)。
- [`completeScheduledStrmMetadata`](../server/managers/PlaybackSessionManager.js:994)：增加第三个 `options` 参数，把 `scheduledTask` 透传给活动任务与结果摘要，供计划任务页区分手动/计划触发。
- [`test/server/utils/bookMetadataFields.test.js`](../test/server/utils/bookMetadataFields.test.js:1)：`normalizeBookMetadataFields`/`isValidBookMetadataFields`/`getBookMetadataFieldLabels` 与 `Scanner.isFieldAllowed`/`Scanner.canOverrideField` 的单测（8 例）。注意 `Scanner` 可以直接 `require` 并调用这两个方法，不需要数据库或全局配置。
- [`client/components/app/ThemeSwitcher.vue`](../client/components/app/ThemeSwitcher.vue:2) 与 [`client/components/widgets/NotificationWidget.vue`](../client/components/widgets/NotificationWidget.vue:2)：顶栏 tooltip 对齐用的 `w-8 h-8 ... mx-1` 容器；改动后需与 [`Appbar.vue`](../client/components/app/Appbar.vue:29) 的统计/上传/设置三个入口保持同一结构。
- [`client/components/widgets/MetadataFieldSelector.vue`](../client/components/widgets/MetadataFieldSelector.vue:1)：本地新增组件。计划任务设置窗的元数据字段内联胶囊开关列表，取代会被窗口底部遮挡的 `ui-multi-select-dropdown`；`value` 为字段数组，`items` 为 `{ value, text }` 候选表，只发 `input` 事件，无弹出层、无全选/清空按钮，默认选中由父级 `normalizeFieldSelection` 保证。
...
- [`client/components/cards/ItemTaskRunningCard.vue`](../client/components/cards/ItemTaskRunningCard.vue:107)：`strmPreloadProgress` 显示媒体预读任务的「已扫描/总数」音轨计数；[`actionIcon`](../client/components/cards/ItemTaskRunningCard.vue:77) 为 `strm-metadata-completion` 使用 `graphic_eq` 图标。
...
2. 保留或重新应用后端 STRM 文件：
   ...
   - 保留书名与作者的提取优先级链：`extractLocalTitle`（书名号）→ `extractSeparatorTitle`（第一个 `丨 | ｜ . ． - － — –` 之前的片段）→ AI 书名+人物 → AI 仅书名 → 全称兜底，入口是 `buildMatchAttempts` 与 `matchLibraryItem` 的逐级循环。**书名必须始终由命中的最高优先级规则决定，AI 不得改写本地已确认的书名**（依赖 `extractSearchMetadata` 的 `confirmedTitle` 机制）。本地规则命中时 AI 只用于补作者栏；AI 未配置或熔断时必须用 `extractLocalAuthor` 的剩余文本/后段文本填作者栏，不能让作者栏变空。未配置 AI 时不能恢复“未配置直接抛错”的前置判断，本地规则与全称兜底必须仍可运行，且候选判断退化为直接采用 provider 首个候选并写入 `matched-local`。`MATCHED_AUDIT_STATUSES` 必须同时包含 `matched-ai` 和 `matched-local`，否则本地匹配成功的书会被计划任务反复重匹配。
...
5. 手工验证三件事：
   ...
   - 分别验证详情页单本补全、选择多本补全（跨书共享暂停计数）和媒体库三点菜单补全都使用同一个媒体库 QPS 且每 3000 文件暂停 5 分钟，并能在任务通知中显示当前书名和进度；修改媒体库 QPS 后新发起的预读立即按新值限速。
   ...
   - 验证书名与作者提取优先级：`《书名》…` 由本地书名号规则确定书名，配置了 AI 时日志显示 `提取规则：书名号+AI 人物`，未配置 AI 时显示 `书名号+剩余文本` 且搜索作者等于书名号以外的文本；`书名丨作者丨主播`、`书名.演播…`、`书名-作者` 由本地符号分隔规则取第一个符号之前的片段，作者栏对应 `符号分隔+AI 人物` 或 `符号分隔+后段文本`（第一个符号之后的全部文本）；带作者搜不到时会自动去掉作者用同一本地书名再搜一次；无书名号与符号标识时先由 AI 提取书名+人物搜索，无结果或未达置信度时删除作者名仅用书名搜索；全部失败或未配置 AI 时用完整原名称搜索。停止计划任务时当前 AI 请求立即取消且不写入取消失败审计。
...
- 入库匹配的唯一耦合点是 [`LibraryItemScanner.scanNewLibraryItem`](../server/scanner/LibraryItemScanner.js:186) 中对 `AiBookMatchManager.enqueueScanMatch` 的一行懒加载调用（避免 `Scanner` → `LibraryScanner` → `LibraryItemScanner` 循环依赖）。上游改写该方法时只需重新插入这一行，队列状态全部保存在 `AiBookMatchManager` 实例字段中，不落库。
...
- 书名与本地作者提取规则全部是纯函数（`extractLocalTitle`、`extractSeparatorTitle`、`extractLocalTitleWithRule`、`extractLocalAuthor`、`trimLocalAuthor`），不依赖数据库或上游模型，可以整段保留；只有 `buildMatchAttempts` 需要在上游改动 `BookFinder.search` 参数签名时同步调整。新增本地规则时在 `TITLE_SEPARATOR_REGEX` 或 `extractLocalTitleWithRule` 内扩展，并同步更新 `MATCH_RULE_LABELS`，不要把规则散落到 `CronManager` 或前端。
