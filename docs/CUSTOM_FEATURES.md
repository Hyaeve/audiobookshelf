...
- [`AiBookMatchManager`](../server/managers/AiBookMatchManager.js:33) 以书籍的 `media.title`（该字段就是原始文件夹名）为输入。本地规则命中时把该名称连同已确认的书名发送给 OpenAI 兼容接口 `/chat/completions`，只让 AI 提取作者和演播者；本地规则都不适用时才让 AI 同时提取书名、作者和演播者；作者与演播者去重合并后分别作为既有 [`BookFinder.search`](../server/finders/BookFinder.js:374) 的标题和作者参数，再获取最多 8 个候选。随后 AI 只能返回本次候选数组中的序号、0 至 1 置信度和理由；服务端拒绝越界序号、非法 JSON 与非法置信度，禁止 AI 自由生成并直接写入元数据。
...
- 书名与作者按固定优先级逐级尝试，任一级搜索并确认匹配成功即结束，不再执行后续级别（[`buildMatchAttempts`](../server/managers/AiBookMatchManager.js:258)、[`matchLibraryItem`](../server/managers/AiBookMatchManager.js:461)）。**书名始终由命中的最高优先级规则决定，AI 只在本地规则命中时负责补作者栏**：
...
- 本地规则命中时**只把 AI 用于提取（演播）作者，绝不让 AI 改写书名**：[`extractSearchMetadata`](../server/managers/AiBookMatchManager.js:299) 会把 [`extractLocalTitleWithRule`](../server/managers/AiBookMatchManager.js:182) 得到的书名作为 `confirmedTitle` 传给 AI，并要求原样复制回 `title`，因此书名号和符号分隔两级的书名恒等于本地提取结果。AI 请求失败时由 [`extractSearchMetadataSafely`](../server/managers/AiBookMatchManager.js:235) 吞掉异常，自动退回本地作者规则，不影响该书继续匹配。
- 未配置 AI 时整条链路只有本地规则（含本地作者）和全称兜底，候选判断退化为直接采用 provider 的首个候选，审计状态记为 `matched-local`。
- 本地作者由 [`extractLocalAuthor`](../server/managers/AiBookMatchManager.js:206) 推导：`quoted` 规则去掉 `《书名》` 整段后取剩余文本，`separator` 规则取第一个分隔符之后的全部文本；两者都用 [`trimLocalAuthor`](../server/managers/AiBookMatchManager.js:190) 去掉首尾的分隔符、空白和标点，并截断到 300 字符。剩余文本为空（例如整名就是 `《天启之门》`）时不追加带作者的尝试。
...
- AI 提取超时或失败时该级只丢掉作者栏（退回本地作者规则或无作者重试），不阻断整本书的匹配。未配置 AI 时 [`runAiBookMatch`](../server/managers/CronManager.js:262) 不再抛出“未配置”错误，任务可以只跑本地规则。
- 计划任务停止通过 [`AbortController`](../server/managers/CronManager.js:273) 中断当前 AI HTTP 请求，并由 [`throwIfAborted`](../server/managers/AiBookMatchManager.js:389) 在每一级 provider 搜索和候选判断前后检查停止状态；取消不会写入 `needs-review` 审计，也不会继续处理下一级或下一本书。
- 计划任务每批读取书籍后先调用 [`getUnmatchedCandidates`](../server/managers/AiBookMatchManager.js:91) 预过滤，只有“除标题、描述和扫描基础信息外，所有扩展元数据均为空”的历史书籍才进入逐本匹配。持续时间、文件大小、音轨、章节、文件路径和 `libraryFiles` 属于扫描基础信息，不影响候选资格。
...
- [`matchLibraryItem`](../server/managers/AiBookMatchManager.js:461) 仍保留同一候选判断作为防御性保护，防止其他调用入口绕过计划任务批次预过滤。
...
- 根因不在提取规则，而在 AI 请求的异常处理。以 `恶魔法则.演播一种侃侃.跳舞.2023` 为例：名称含 `.`，本地符号分隔规则正常命中并提取出 `恶魔法则`，provider 也返回了候选；随后 [`chooseCandidate`](../server/managers/AiBookMatchManager.js:346) 请求 AI 接口拿到 `503`，`axios` 抛出的异常一路冒泡出 [`matchLibraryItem`](../server/managers/AiBookMatchManager.js:461)，被 [`CronManager`](../server/managers/CronManager.js:307) 的 `catch` 兜住后统一写成 `needs-review`。该 `catch` 构造的 `matchResult` 只有 `status` 和 `reason`，没有 `rule`/`searchTitle`/`searchAuthor`，所以日志把这三项打成 `-`，造成“规则没生效”的误判。入库匹配走 [`processScanMatchQueue`](../server/managers/AiBookMatchManager.js:574) 的 `catch`，表现为“入库匹配失败”。
- 修复一：AI 请求统一走带重试的 [`postAiRequest`](../server/managers/AiBookMatchManager.js:424)。`408`、`429`、`5xx` 和网络层错误（无 `response`）最多重试 3 次；有 `Retry-After` 头时按该值等待（上限 30 秒），否则按 2 秒、4 秒递增。`401`、`403`、`400` 一类确定性错误立即失败，不做无意义重试。等待走 [`wait`](../server/managers/AiBookMatchManager.js:403)，可被停止信号打断。
- 修复二：AI 失败不再让整本书失败。[`buildMatchAttempts`](../server/managers/AiBookMatchManager.js:258) 的书名提取失败原本已经降级，本次补上候选判定：`matchLibraryItem` 内的 `chooseCandidate` 失败后只把本书剩余级别的 `aiUsable` 置为 `false`，直接采用 provider 首个候选并把审计写成 `matched-local` / `source: local`，而不是抛出异常。审计里的 `status`/`source`/`model` 改为依据实际是否用到 AI 的 `usedAi` 变量，而不是“是否配置了 AI”的 `aiConfigured`，避免降级后仍被标成 `matched-ai`。
- 修复三：新增软熔断。连续 `AI_FAILURE_STREAK_LIMIT = 3` 次 AI 传输失败后，[`noteAiFailure`](../server/managers/AiBookMatchManager.js:72) 把 `aiUnavailableUntil` 推后 `AI_COOLDOWN_MS = 5` 分钟，期间 [`isAiUsable`](../server/managers/AiBookMatchManager.js:57) 返回 `false`，整批书籍跳过 AI 步骤、只用本地规则继续匹配，不会每本书都白等三次重试超时。任一次 AI 成功即由 [`noteAiSuccess`](../server/managers/AiBookMatchManager.js:61) 清零。熔断状态只存在于 `AiBookMatchManager` 实例字段，不落库、不影响 `isConfigured` 的语义。
- 取消判定集中到 [`isCancelledError`](../server/managers/AiBookMatchManager.js:393)（检查 `options.signal.aborted`、`ERR_CANCELED`、`CanceledError`），停止计划任务时仍立即中断且不写失败审计、不触发熔断计数以外的重试。
- 修复四：兜底日志不再丢信息。[`CronManager`](../server/managers/CronManager.js:307) 的 `catch` 和 [`processScanMatchQueue`](../server/managers/AiBookMatchManager.js:574) 的 `catch` 都会重新用 `extractLocalTitleWithRule` 算出规则与书名后再写审计和日志，因此即便出现未预料的异常，日志也会显示真实的提取规则和搜索标题，而不是三个 `-`。
...
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:11)：`MATCHED_AUDIT_STATUSES` 把 `matched-ai` 与本地匹配成功状态 `matched-local` 一并视为已匹配，供 [`isUnmatchedCandidate`](../server/managers/AiBookMatchManager.js:101) 判断。
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:13)：`TITLE_SEPARATOR_REGEX` 定义第二级本地规则识别的分隔符 `丨 | ｜ . ． - － — –`；紧随其后的 [`LOCAL_AUTHOR_TRIM_REGEX`](../server/managers/AiBookMatchManager.js:15) 负责裁掉本地作者文本首尾的分隔符、空白和标点。
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:21)：`MATCH_RULE_LABELS` 提取规则的中文日志名称（书名号、书名号+AI 人物、书名号+剩余文本、符号分隔、符号分隔+AI 人物、符号分隔+后段文本、AI 书名+人物、AI 仅书名、全称）。
- [`server/managers/AiBookMatchManager.js`](../server/managers/AiBookMatchManager.js:34)：构造函数中的 `apiRouterCtx`、`scanMatchQueue`、`scanMatchQueuedIds`、`scanMatchRunning` 四个入库匹配队列字段，全部只存在于内存，不落库。
- [`extractLocalTitle`](../server/managers/AiBookMatchManager.js:157)：第一级书名号提取，保持原有实现不变。
- [`extractSeparatorTitle`](../server/managers/AiBookMatchManager.js:162)：第二级符号分隔提取，取第一个分隔符之前的非空片段；整名没有分隔符或首段等于原名时返回空串。
- [`extractLocalTitleWithRule`](../server/managers/AiBookMatchManager.js:182)：把两条本地规则合并为 `{ title, rule }`，书名号优先于符号分隔。
- [`trimLocalAuthor`](../server/managers/AiBookMatchManager.js:190) 与 [`extractLocalAuthor`](../server/managers/AiBookMatchManager.js:206)：未配置 AI 或 AI 不可用时的本地作者推导。`quoted` 去掉 `《书名》` 整段取剩余文本，`separator` 取第一个分隔符之后的全部文本，其他规则返回空串。
- [`extractSearchMetadataSafely`](../server/managers/AiBookMatchManager.js:235)：包裹 `extractSearchMetadata`，吞掉传输失败并记入熔断计数，只有取消信号才继续向上抛。
- [`getMatchRuleLabel`](../server/managers/AiBookMatchManager.js:222)：规则英文键到中文标签的映射，供日志使用。
- [`buildMatchAttempts`](../server/managers/AiBookMatchManager.js:258)：生成有序尝试列表。本地规则命中时书名固定为本地结果，先产出带作者的一级（AI 可用取 `*-ai-author`，否则取 `*-local-author`），再产出无作者的同名一级；本地规则都不命中且 AI 可用时才调用 [`extractSearchMetadata`](../server/managers/AiBookMatchManager.js:299) 产出 `ai-title-author`、`ai-title` 两级；最后无条件追加 `full-name`，并按“标题+作者”去重。
- [`postAiRequest`](../server/managers/AiBookMatchManager.js:424)：所有 AI 请求的唯一出口，重试 408/429/5xx 与网络错误（最多 3 次，遵守 `Retry-After`），确定性错误立即失败。
- [`isCancelledError`](../server/managers/AiBookMatchManager.js:393) 与 [`wait`](../server/managers/AiBookMatchManager.js:403)：统一的取消判定和可打断退避等待。
- [`isAiUsable`](../server/managers/AiBookMatchManager.js:57) / [`noteAiSuccess`](../server/managers/AiBookMatchManager.js:61) / [`noteAiFailure`](../server/managers/AiBookMatchManager.js:72)：AI 软熔断（连续 3 次传输失败后暂停 5 分钟，只用本地规则），常量 `AI_FAILURE_STREAK_LIMIT`、`AI_COOLDOWN_MS` 位于文件顶部。
- [`throwIfAborted`](../server/managers/AiBookMatchManager.js:389)：统一的停止信号检查点，替代原来散落的多处 `options.signal?.aborted` 判断。
- [`matchLibraryItem`](../server/managers/AiBookMatchManager.js:461)：逐级执行尝试列表。每级先 provider 搜索，无候选就进入下一级；已配置 AI 时由 [`chooseCandidate`](../server/managers/AiBookMatchManager.js:346) 把关且未达阈值继续下一级，未配置 AI 时直接取首个候选。成功后写 `matched-ai`/`matched-local` 审计并返回 `rule`、`ruleLabel`；全部失败按最后一次失败原因写 `unmatched`/`needs-review`。
- [`enqueueScanMatch`](../server/managers/AiBookMatchManager.js:561)：入库匹配入队。校验 `aiBookMatchOnScan`、`mediaType === 'book'`、`aiBookMatchLibraryIds`（为空表示不限制）与书籍 ID 去重，任一不满足直接返回 `false`，因此关闭该选项时对上游扫描流程零影响。
- [`processScanMatchQueue`](../server/managers/AiBookMatchManager.js:574)：串行处理队列，一次只匹配一本书；先轮询 [`LibraryScanner.isLibraryScanning`](../server/scanner/LibraryScanner.js:33) 等待所属媒体库扫描结束，再用 `getExpandedById` 重新读取书籍并调用 `matchLibraryItem`，单本失败只记警告不中断队列。
- [`setApiRouterContext`](../server/managers/AiBookMatchManager.js:609)：接收 `ApiRouter` 实例，供入库匹配复用 `checkRemoveAuthorsWithNoBooks` 与 `checkRemoveEmptySeries`。
