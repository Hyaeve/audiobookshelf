...
- 只有播放窗口中的真实目标才会执行时长探测：本地目标使用真实路径，远程目标使用已缓存的内存 Buffer 通过 `ffprobe` 标准输入探测；扫描阶段和窗口外章节都不会触发探测。
...
- 已探测章节的真实 `duration` 会写入当前会话并重建 `startOffset`，不会回写扫描占位数据；由于启动时最多访问十章，全书总时长会在窗口滚动和浏览器加载 metadata 后逐步校正。
...
2. 保留或重新应用后端 STRM 文件：
   ...
   - 保留会话启动时的十章真实预取、播放阶段时长探测、内存响应缓存、本地句柄缓存和关闭释放逻辑。
