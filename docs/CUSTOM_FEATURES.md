...
- 完整扫描严格串行请求，并按本书 STRM 文件总数限速：少于 800 个为 1.0 QPS，800 至 2000 个为 0.8 QPS，超过 2000 个为 0.5 QPS。已有完整音轨信息的目标不会重复请求。
...
- 页面提供“补全元数据”和“清理丢失项目”两条横条；每条右侧都有立即执行图标和竖三点设置入口。
- 补全元数据支持 cron 表达式和单次最长执行时间，时间从 0.5 小时开始、按 0.5 小时递增；服务端校验 cron 和步长。
- 补全元数据只处理缺少有声书总时长的书籍，已有持续时长的书籍跳过；执行限速固定为 0.5 QPS，每处理 5000 个文件暂停 5 分钟，并受单次小时数截止时间限制。
...
- 背景使用深邃藏蓝、墨紫和炭黑底色。
- 增加错落大小的银白、浅蓝、淡金和浅紫星点，并通过缓慢漂移与透明度动画形成闪烁星光；背景层不阻挡页面交互。
...
2. 保留或重新应用后端 STRM 文件：
   ...
   - 保留完整扫描的串行 QPS 限速：少于 800 个为 1.0 QPS，800 至 2000 个为 0.8 QPS，超过 2000 个为 0.5 QPS。
...
5. 手工验证三件事：
   ...
   - 分别用 799、800、2000、2001 个 STRM 文件验证 QPS 为 1.0、0.8、0.8、0.5。
   - 在设置侧栏用户下方验证计划任务入口；分别手动执行两条任务，确认清理任务只删除 `isMissing` 数据库项目，不删除文件，也不删除仅 `isInvalid` 的项目。
   - 切换浩瀚星空主题，确认藏蓝/墨紫/炭黑背景、不同颜色和大小星点、缓慢移动与闪烁效果在桌面和移动端均可见且不遮挡交互。
