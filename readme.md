...
播放章节时，服务端只解析并代理当前请求的 `.strm` 目标，不提前请求固定数量的其他章节。远程播放透传 HTTP Range，本地目标直接读取已挂载文件。

一本 STRM 有声书被请求播放后，若任一音轨缺少真实时长、编码或声道信息，服务端会在返回播放响应后启动整本书完整扫描：

- 完整扫描逐个请求尚未补全的目标，不并发请求。
- 本书 STRM 文件总数少于 800 时限制为 1.0 QPS；800 至 2000 个时限制为 0.8 QPS；超过 2000 个时限制为 0.5 QPS。
- 扫描成功后持久化音轨时长、编码、码率、声道和标签，并重建书籍章节及总时长。
- 全部音轨和书籍聚合元数据完整后，后续播放不会再次执行完整扫描。
- 本地目标必须位于允许的媒体库目录或固定挂载根目录 `/NetDisk` 内。

## 主题切换

顶部工具栏提供主题切换按钮，支持经典主题和星空夜间主题。主题选择保存在浏览器本地，刷新页面后仍然有效。
